...
import { MetadataFetcher, type MetadataFetchJob } from '../services/metadata-fetcher.js';
...
const metadataRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/fetch', async (request, reply) => {
    ...
    const { videoIds } = body; // Optional: specific video IDs to fetch
      ...
      .then(async (results) => {
        ...
        for (const [videoId, result] of results) {
          const updateData: any = {
            hasCompleteMetadata: result.parsedMetadata.hasCompleteMetadata,
          ...
          if (result.parsedMetadata.durationSeconds) {
            updateData.durationSeconds = result.parsedMetadata.durationSeconds;
          }
          ...
          if (result.thumbnailPath && result.thumbnailPath !== jobs.find(j => j.videoId === videoId)?.thumbnailPath) {
