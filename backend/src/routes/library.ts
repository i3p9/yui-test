...
const libraryRoutes: FastifyPluginAsync = async (fastify) => {
	...
	fastify.get("/latest", async (request, reply) => {
		...
		const [videos, total] = await Promise.all([
			prisma.video.findMany({
				...
				skip,
				take: Number(limit),
				orderBy: [{ uploadDate: "desc" }, { lastScannedAt: "desc" }],
				select: {
					videoId: true,
					title: true,
					uploader: true,
		...
		async (request, reply) => {
			...
			try {
				...
				if (!channel || !channel.thumbnailPath) {
					return reply
						.code(404)
						.send({ error: "Channel thumbnail not found" });
				}
				...
				try {
					...
					return reply
						.code(404)
						.send({ error: "Thumbnail file not found" });
				}

				// Determine content type based on file extension
				const ext = channel.thumbnailPath
					.toLowerCase()
					.split(".")
					.pop();
				const contentType =
				...
				console.error(
					`Error serving channel thumbnail for ${uploaderId}:`,
					error,
				);
				return reply
					.code(500)
					.send({ error: "Failed to serve thumbnail" });
			}
		},
	);
		...
		async (request, reply) => {
			...
			const [videos, total, channel] = await Promise.all([
				prisma.video.findMany({
					...
					take: Number(limit),
					orderBy: {
						[sortField]: sortField === "title" ? "asc" : "desc",
					},
					select: {
						videoId: true,
						title: true,
	...
	fastify.get("/liked", async (request, reply) => {
		const query = request.query as any;
		const { page = 1, limit = 20, orderBy = "uploadDate" } = query;
	...
	fastify.get("/video/:videoId/related", async (request, reply) => {
		...
		if (
			currentVideo.mediaType === "channel_archive" &&
