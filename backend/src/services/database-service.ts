...
import type { ParsedMetadata, Library } from '../types/index.js';
...
export class DatabaseService {
   ...
   */
  async upsertVideo(metadata: ParsedMetadata, library: Library): Promise<void> {
    ...
    if (existing) {
      ...
      const shouldReplace = await this.shouldReplaceVideo(existing, metadata);

      if (shouldReplace) {
        console.log(`Replacing ${metadata.videoId} with larger/better file`);
  ...
  private async insertVideo(metadata: ParsedMetadata): Promise<void> {
    ...
    await this.prisma.video.create({
      data: {
        ...
        filesizeBytes: metadata.filesizeBytes ? BigInt(metadata.filesizeBytes) : null,
